...
# A user-role message generated by the app itself rather than typed by the
# user. It is replayed to the model as a normal user message, but rendered in
# the chat UI as a small notice instead of a "you" bubble.
AUTO_USER_MARKER = "__auto__"
    ...
    *,
    ...
    if attachments:
        ...
        m = models.Message(
            ...
            role="user",
            content=text,
            name=AUTO_USER_MARKER if auto else None,
        )
    db.add(m)
    db.commit()
    db.refresh(m)
