...
- **Interrupted runs recover.** A run's live subprocess dies with the backend, so a crash or restart would otherwise leave its row stuck reading `running` — uncancellable, undeletable, and blocking deletion of its workflow. Such rows are reconciled to a terminal state on startup, and cancelling a run with no live subprocess marks it `cancelled` rather than hanging, so a run is never permanently stuck.
